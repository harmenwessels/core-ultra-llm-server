"""Interleaved engine A/B: decode speed + output identity for the same IR under two venvs.
    ...
    python engine_ab.py <venv_a> <venv_b> <model_dir> [rounds=3] [device=GPU]

Runs A, B, A, B, … in fresh processes (one pipeline per process, its own CACHE_DIR
per venv so compile blobs never mix), greedy, a fixed ~200-token coding prompt.
...
venv, model, device = sys.argv[1], sys.argv[2], sys.argv[3]
...
def run(venv: str, model: str, device: str) -> dict:
    py = f"{venv}\\Scripts\\python.exe"
    cache = f".ovcache-ab-{venv.strip('.').replace('/', '_')}"
    out = subprocess.run([py, "-c", CHILD, venv, model, device, cache], capture_output=True, text=True,
...
def main() -> None:
    ...
    print(f"model={model} device={device} rounds={rounds}  A={a}  B={b}", flush=True)
    res = {a: [], b: []}
    wins_b = 0
    for i in range(rounds):
        for v in (a, b):
            r = run(v, model, device); res[v].append(r)
    ...
    print(f"  greedy output identical across engines: {same}")
