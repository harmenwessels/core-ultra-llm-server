...
[benchmark/](benchmark/README.md) for the clean per-task-type leaderboard (GenAI-only, quality +
time-to-solve, provenance-logged) and the single-vs-combo model recommendations.
...
lives in [benchmark/](benchmark/README.md)**; the underlying findings and conversion playbook in
[RESEARCH.md](RESEARCH.md).

### Best setup per task type (generated from the benchmark)

The best single model and best `virtual/agent` combo for each task type, regenerated by
`benchmark/scripts/assemble_leaderboard.py` from the provenance run-records:

<!--BEST-SETUP START-->
| Task type | Best single | Q | Best combo | Q |
|---|---|---|---|---|
| analysis | OpenVINO/Qwen2.5-Coder-1.5B-Instruct-int4-ov | 3/4 | — | — |
| autocomplete-fim | OpenVINO/Qwen2.5-Coder-1.5B-Instruct-int4-ov | 1/1 | — | — |
